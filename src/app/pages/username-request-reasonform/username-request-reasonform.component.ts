...
export class UsernameRequestReasonformComponent implements OnInit {
  ...
  reason;
  constructor(
    public dialog: MatDialogRef<UsernameRequestReasonformComponent>,
  ...
  requestToAdminForReservedUserName() {
    ...
    this.service.apiPOST("requestToAdminForReservedUserName", data).subscribe(
      (result: any) => {
        this.show = false;
        ...
        if (result.status == true) {
          this.dialogService.showAlert(
            "success",
            "Success!",
            "Your request will be process in the next 24 hours. A random username will be generated to you. Username will be updated automatically after admin approved it."
          );
          this.dialog.close();
